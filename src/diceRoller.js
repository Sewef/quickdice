...
function parseInput(userInput) {
    ...
    const keywords = ['save', 'load', 'delete'];
    const keywordPattern = /(save|load|delete)/g;
    let match;
    ...
    function extractDiceExpressions(text) {
        ...
        while ((match = regex.exec(text)) !== null) {
            if (match[2] !== undefined && match[3] !== undefined) {
                ...
                // Match d'un simple nombre (ex: +3, -5)
                const operator = match[4] === '-' ? '-' : '+';
                const modifier = parseInt(match[5], 10);
    ...
    const result = extractDiceExpressions(cleanedUserInput);
    console.log(result);
