...
export function setupDiceRoller() {
    const diceBox = new DiceBox({
        ...
        container: '#dice-box',    // DOM element to place the canvas
...
function parseInput(userInput) {
    ...
    function parseAttackRoll(text) {
        const attackRollPattern = /^(\d+)([nad])((?:[+-](?:\d+d\d+|\d+))*)$/i;
        ...
        const attack_bonus_str = match[3];
...
function getThemeAndColor(damageType) {
    const mapping = {
        ...
        'ne': { theme: 'smooth', themeColor: '#2f4f4f' }, // Necrotic
...
async function performAttack(attackParams, diceBox) {
    ...
    // Handle null damage_components
    if (attackParams.damage_components === null) {
        damageResults = null;
    }
    ...
    if (attackParams.num_attacks === null) {
    ...
    for (let i = 0; i < attackParams.num_attacks; i++) {
        if (!automaticHit) {
            ...
            if (attackParams.modifier === 'a') { // Advantage
                // Roll two d20s
                attackDice.dice.push({ type: 'd20', count: 1, description: `Attack ${i + 1} roll1` });
                attackDice.dice.push({ type: 'd20', count: 1, description: `Attack ${i + 1} roll2` });
                ...
                // Roll one d20
                attackDice.dice.push({ type: 'd20', count: 1, description: `Attack ${i + 1} roll` });
            }
    ...
    diceRolls.attackDice = await executeDiceRolls(diceRolls.attackDice, true, diceBox);
    ...
    diceRolls.damageDice = await executeDiceRolls(diceRolls.damageDice, true, diceBox);
